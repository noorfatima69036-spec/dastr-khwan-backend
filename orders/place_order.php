...
// STRICT USER LOGIN CHECK
if (!$user_id || $user_id <= 0) {
    echo json_encode(["success" => false, "message" => "Authentication failed. Please login to place an order."]);
    exit();
}

if (!$full_name || !$phone || count($items) === 0) {
...
// 1. Order insert into 'orders' table
...
// 2. Order items insert into 'order_items' table
...
foreach ($items as $item) {
    ...
    $price = $item['price'];
    $portionType = $item['portion_type'] ?? 'full';
